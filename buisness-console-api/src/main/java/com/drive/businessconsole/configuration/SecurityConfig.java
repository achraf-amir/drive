package com.drive.businessconsole.configuration;

import com.drive.businessconsole.handler.GoogleSuccessHandler;
...
@Autowired
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http
						...
						.antMatchers("/", "/**.html", "/**.js", "/google/login", "/**/favicon.ico").permitAll()
						...
						.addFilterAfter(filter(googleSuccessHandler()), SecurityContextPersistenceFilter.class)
						.csrf()
	...
	private OAuth2ClientAuthenticationProcessingFilter filter(AuthenticationSuccessHandler successHandler) {
	...
	@Bean
	public OAuth2ClientContext requestContextListener() {
	...
	@Bean // handles the redirect to google
	public FilterRegistrationBean oAuth2ClientFilterRegistration(OAuth2ClientContextFilter filter) {
		FilterRegistrationBean registration = new FilterRegistrationBean();
		registration.setFilter(filter);
		registration.addUrlPatterns("/google/login");
